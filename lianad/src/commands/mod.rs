...
use liana::{
    ...
    spend::{
        ...
        SpendState, SpendStatus, SpendTxFees, TxGetter, TxMempoolState,
...
impl DaemonControl {
    ...
    /// What a transaction must pay to replace the transactions spending these outpoints in the
    /// mempool.
    ///
    /// Returns `None` if no transaction in the mempool is spending any of them.
    fn replacement_requirements(
        &self,
        outpoints: &[bitcoin::OutPoint],
    ) -> Option<ReplacementRequirements> {
        let entries = self.bitcoin.mempool_spenders(outpoints);
        if entries.is_empty() {
            return None;
        }
    ...
    ///
    /// `db_coins` holds at least the coins this psbt spends, so the caller can read them for
    /// several psbts at once.
    fn spend_state(
        &self,
        psbt: &Psbt,
        ...
        let outpoints: Vec<bitcoin::OutPoint> = psbt
            .unsigned_tx
            .input
            .iter()
            .map(|txin| txin.previous_output)
            .collect();
        let coins: Vec<Option<SpendCoin>> = outpoints
            .iter()
            .map(|outpoint| {
                db_coins.get(outpoint).map(|coin| SpendCoin {
                    amount: coin.amount,
            ...
            .collect();
        let mempool = if self.bitcoin.mempool_entry(&txid).is_some() {
            TxMempoolState::Broadcast
        } else {
            match self.replacement_requirements(&outpoints) {
                Some(requirements) => TxMempoolState::Conflicting(requirements),
                None => TxMempoolState::Broadcastable,
            }
        };
        psbt_spend_state(
            psbt,
    ...
    pub fn list_spend(
        ...
        let spend_txs = spend_psbts
            .into_iter()
            .map(|(psbt, updated_at)| {
                let state = self.spend_state(&psbt, &db_coins, tip_height);
                ListSpendEntry {
                    psbt,
                    updated_at,
                    status: state.status,
    ...
    pub fn rbf_psbt(
        ...
        let ReplacementRequirements {
            ...
            .replacement_requirements(&prev_outpoints)
