...
pub fn account_select_view(state: &State) -> Element<'_, Msg> {
    ...
    let new_email_row = Row::new()
        ...
        .width(Length::Fill)
        .push(new_email)
        .push(Space::with_width(Length::Fixed(list::ACCOUNT_DELETE_SLOT_WIDTH)));
...
fn account_entry(
    ...
    list::account_entry(
        email,
        (!processing).then(|| Msg::AccountSelectConnect(account.email.clone())),
        (!processing).then(|| Msg::AccountSelectDelete(account.email.clone())),
    )
}
