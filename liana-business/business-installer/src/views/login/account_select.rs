use crate::{
    state::{Msg, State},
    views::{delete_btn, layout_with_scrollable_list, INSTALLER_STEPS},
};
use iced::{
    widget::{row, Space},
    Alignment, Length,
...
pub fn account_select_view(state: &State) -> Element<'_, Msg> {
    ...
    for account in accounts {
        ...
        let trailing = Some(delete_btn(delete_msg).into());

        list_content = list_content.push(list::entry_account(
            title,
            None::<String>,
            trailing,
            card_message,
