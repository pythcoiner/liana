...
use liana_ui::{
    component::{
        button::{btn_primary, btn_tertiary, BtnWidth},
        modal::ModalWidth,
        text,
    },
    icon, theme,
...
pub fn template_help_modal_view(_modal_state: &TemplateHelpModalState) -> Element<'_, Msg> {
    ...
    let footer = row![
        ...
        btn_tertiary(
            ...
            "Close",
            BtnWidth::M,
            Some(Msg::TemplateHelpCloseModal),
        ),
        btn_primary(
            None,
            "Email WS",
            BtnWidth::Auto,
            Some(Msg::TemplateHelpEmailWs),
        ),
        Space::fill_width()
    ]
    .spacing(10);

    let close = Button::new(icon::cross_icon().size(40))
        .padding(0)
        .style(theme::button::transparent)
        .on_press(Msg::TemplateHelpCloseModal);
