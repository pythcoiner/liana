...
fn signer_field<'a>(state: &'a State, modal_state: &'a EditKeyModalState) -> Element<'a, Message> {
    ...
    let signer_picker = column![combobox::editable_menu_combobox(
        ...
        EditableMenuActions {
            ...
            on_open: Some(Message::KeyOpenSignerMenu),
            on_close: Some(Message::KeyEmailBlurred),
        },
    )];
    let invalid_email: Option<Element<'_, Message>> =
        (!is_empty && !email_valid && modal_state.email_blurred).then(|| {
