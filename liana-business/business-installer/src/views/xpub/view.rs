...
fn xpub_key_card(
    ...
    let title = row![
        ...
        compact_kind_pill(&key.key_type)
    ]
    .spacing(10)
    ...
    list::list_entry_row(
        Some(badge::tile(key_tile(&key.key_type)).into()),
        body,
        Some(status),
...
fn compact_kind_pill(key_type: &KeyType) -> Element<'static, Msg> {
    pill::compact_pill(
        key_kind_label(key_type),
        pill::PillWidth::Shrink,
        key_pill_style(key_type),
    )
    .into()
}

fn key_pill_style(key_type: &KeyType) -> fn(&theme::Theme) -> iced::widget::container::Style {
    match key_type {
        KeyType::Internal => theme::pill::internal,
        KeyType::External => theme::pill::external,
        KeyType::Cosigner | KeyType::SafetyNet => theme::pill::safety_net,
    }
}

fn key_tile(key_type: &KeyType) -> TileName {
    match entry_key_kind(key_type) {
        list::EntryKeyKind::Internal => TileName::KeyInternal,
        list::EntryKeyKind::External => TileName::KeyExternal,
        list::EntryKeyKind::SafetyNet => TileName::KeyService,
    }
}
...
pub fn xpub_view(state: &State) -> Element<'_, Msg> {
    ...
    let header_content = screen_intro("Set Keys", Some(intro_description(instruction)));
