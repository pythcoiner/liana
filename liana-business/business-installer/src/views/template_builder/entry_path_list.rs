    ...
    path.key_ids
        ...
        .map(|key| {
            pill::compact_pill(
                ...
                match entry_key_kind(&key.key_type) {
                    list::EntryKeyKind::Internal => theme::pill::internal,
                    list::EntryKeyKind::External => theme::pill::external,
                    list::EntryKeyKind::SafetyNet => theme::pill::safety_net,
                },
            )
            .into()
        })
...
fn availability_pill(timelock: Option<&ws_business::Timelock>) -> Element<'static, Msg> {
    match timelock {
        Some(timelock) => pill::pill_with_icon(
            Some(icon::clock_icon().style(theme::text::secondary)),
            format_timelock_human(timelock),
            "",
            pill::PillWidth::Shrink,
