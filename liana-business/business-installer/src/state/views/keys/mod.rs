...
impl KeysViewState {
    ...
    pub fn on_key_update_email(&mut self, value: String) {
        ...
        if let Some(modal) = &mut self.edit_key_modal {
            ...
            modal.open_signer_combobox();
        }
    }

    pub fn on_key_open_signer_menu(&mut self) {
        if let Some(modal) = &mut self.edit_key_modal {
            modal.open_signer_combobox();
        }
    }

    pub fn on_key_email_blurred(&mut self) {
        if let Some(modal) = &mut self.edit_key_modal {
            modal.email_blurred = true;
        }
    }
...
mod tests {
    ...
    fn clone_keeps_modal_runtime_state() {
        ...
        assert!(modal.email_blurred);
        assert!(modal.signer_combobox.is_open());
    }
}
