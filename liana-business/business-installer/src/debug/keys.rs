...
use liana_connect::ws_business::{Key, KeyIdentity, KeyType as WsKeyType, UserRole};
...
/// already come from `seed_test_data`).
fn extend_with_cosigner_and_safety_net(s: &mut State) {
    let mut template = s.app.template();
    ...
    s.app.apply_server_template(template);
}
...
fn extend_with_many_keys(s: &mut State) {
    ...
    let entries: &[(WsKeyType, &str, KeyIdentity)] = &[
        (
            WsKeyType::Internal,
            "Walaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa   let manager",
            KeyIdentity::Email("owner@example.com".to_string()),
        ),
        (
            WsKeyType::External,
            "Aliceeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",
            KeyIdentity::Email("alice@example.com".to_string()),
        ),
        (
            WsKeyType::External,
            "Bobiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii",
    ...
    for (next, (key_type, alias, identity)) in entries.iter().enumerate() {
        let next = next as u8;
        template.keys.insert(
            next,
            Key {
...
/// Wallet Manager, keys marked ready: the list is read-only, so rows show the
/// `in_policy` / `not_in_policy` pills (the seeded keys are in spending paths;
/// one extra key is not) and the footer is the "Edit keys" / success notice.
fn keys_ready_state() -> &'static State {
    static S: OnceLock<StateCell<State>> = OnceLock::new();
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            s.current_view = View::Keys;
            add_sample_org_and_wallet(s);
            s.app.current_user_role = Some(UserRole::WalletManager);
            let mut template = s.app.template();
            let unused_id = s.app.next_key_id();
            template.keys.insert(
                unused_id,
                Key {
                    id: unused_id,
                    alias: "Unassigned key".to_string(),
                    description: String::new(),
                    identity: KeyIdentity::Email("unused@example.com".to_string()),
                    key_type: WsKeyType::External,
                    xpub: None,
                    xpub_source: None,
                    xpub_device_kind: None,
                    xpub_device_version: None,
...
fn render_keys_ready() -> Element<'static, DebugMessage> {
    let body = keys_view(keys_ready_state()).map(|_| ());
    installer_chrome(
        "Business installer — keys (manager, ready / read-only)",
        KEYS_PATH,
        body,
    )
}

/// Wallet Manager still setting up, with a single key: the footer shows the
/// "Add at least 2 keys to continue" hint next to the disabled "Mark keys
/// ready" button, plus the info notice card.
fn keys_manager_setup_state() -> &'static State {
    static S: OnceLock<StateCell<State>> = OnceLock::new();
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            s.current_view = View::Keys;
            ...
            s.app.current_user_role = Some(UserRole::WalletManager);
            let mut template = s.app.template();
            template.keys.retain(|id, _| *id == 0);
            template.keys_ready = false;
            s.app.apply_server_template(template);
        }))
    })
    .0
...
fn render_keys_manager_setup() -> Element<'static, DebugMessage> {
    let body = keys_view(keys_manager_setup_state()).map(|_| ());
    installer_chrome(
        "Business installer — keys (manager, awaiting ready)",
        KEYS_PATH,
        body,
    )
}

pub static ENTRY_KEYS_READY: DebugPageEntry = DebugPageEntry {
    view: render_keys_ready,
};
pub static ENTRY_KEYS_MANAGER_SETUP: DebugPageEntry = DebugPageEntry {
    view: render_keys_manager_setup,
};
...
fn build_key_modal_state(
    ...
    build_state(|s| {
        s.current_view = View::Keys;
        let mut modal = EditKeyModalState::new(
            if is_new { 99 } else { 1 },
            alias.to_string(),
        ...
        modal.token_warning = token_warning;
        s.views.keys.edit_key_modal = Some(modal);
    })
}
...
fn key_modal_external_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_key_modal_state(
            false,
            WsKeyType::External,
            "Bobaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            "",
            "bob@example.com",
            "",
            None,
...
fn key_modal_internal_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_key_modal_state(
            false,
            WsKeyType::Internal,
            "Waaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaallet manager",
            "",
            "owner@example.com",
            "",
            None,
