...
fn org_select_with_orgs_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            let names = [
                "Acme Vault",
                "Treasury Coooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo",
                "Cold Storage Inc.",
            ];
...
pub static ENTRY_ORG_SELECT_EMPTY: DebugPageEntry = DebugPageEntry {
    view: render_org_select_empty,
};

/// No organizations: `org_select_view` renders its empty placeholder.
fn org_select_empty_state() -> &'static State {
    static S: OnceLock<StateCell<State>> = OnceLock::new();
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            s.app.global_user_role = Some(UserRole::WizardSardineAdmin);
...
fn render_org_select_empty() -> Element<'static, DebugMessage> {
    let body = org_select_view(org_select_empty_state()).map(|_| ());
    installer_chrome(
        "Business installer — org select (empty)",
        ORG_SELECT_PATH,
        body,
    )
}
