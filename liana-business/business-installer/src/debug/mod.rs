...
use liana_connect::ws_business::{
    Key, KeyIdentity, KeyType, Org, PolicyTemplate, SecondaryPath, SpendingPath, Timelock, User,
    UserRole, Wallet, WalletStatus,
};
use liana_gui::{
...
use crate::state::{
    app::AppState,
    views::modals::{ConflictModalState, ConflictType, TemplateHelpModalState, WarningModalState},
    State,
};
use crate::views::{
    modals::conflict::conflict_modal_view, modals::template_help::template_help_modal_view,
    modals::warning::warning_modal_view,
};
...
pub(super) fn build_state(setup: impl FnOnce(&mut State)) -> State {
    ...
    seed_test_data(&mut s.app);
    // Populate the logged-in email so the user-header bar renders it; pages that
    // need a different (or empty) email override it in `setup`.
    s.views.login.on_update_email("alice@example.com".to_string());
    setup(&mut s);
    s
}

/// Pre-populate `app` with three keys (Wallet Manager, Bob, Alice), a
/// 2-of-2 primary path, and two timelocked secondary paths. Used by every
/// state-based debug page so views render against realistic data without
/// every caller having to assemble the same fixture.
fn seed_test_data(app: &mut AppState) {
    let mut keys = BTreeMap::new();
    keys.insert(
        0,
        Key {
            id: 0,
            alias: "Wallet Manageeeeeeeeeeeeeeeeeeeeeeeeeeeeeer".to_string(),
            description: String::new(),
            identity: KeyIdentity::Email(
                "owner@exaaaaaaaaaaaaaaaaaaaaaaaaaaaaaample.com".to_string(),
            ),
            ...
            xpub_file_name: None,
            last_edited: Some(1779000198),
            last_editor: Some(Uuid::from_u128(0x1234)),
        },
    );
    keys.insert(
        1,
        Key {
            ...
            description: String::new(),
            identity: KeyIdentity::Email(
                "boooooooooooooooooooooooooooooooooooooooooooooooob@example.com".to_string(),
            ),
    ...
    keys.insert(
        ...
        Key {
            ...
            description: String::new(),
            identity: KeyIdentity::Email("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaalice@eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeexample.com".to_string()),
            key_type: KeyType::External,
            xpub: None,
            xpub_source: None,
            xpub_device_kind: None,
    ...
    app.apply_server_template(PolicyTemplate {
        keys,
        primary_path: SpendingPath::new(true, 2, vec![0, 1]),
        secondary_paths: vec![
            SecondaryPath {
                path: SpendingPath::new(false, 1, vec![2, 1]),
...
pub(super) fn add_sample_org_and_wallet(s: &mut State) {
    s.backend.users.lock().expect("poisoned").insert(
        Uuid::from_u128(0x1234),
        User {
            name: "John Doe".to_string(),
            uuid: Uuid::from_u128(0x1234),
            email: "john.doe@123.4".to_string(),
...
pub static ENTRY_CONFLICT_MODAL_PATH_CHOICE: DebugPageEntry = DebugPageEntry {
    view: render_conflict_modal_path_choice,
};
pub static ENTRY_CONFLICT_MODAL_PATH_DELETED: DebugPageEntry = DebugPageEntry {
    view: render_conflict_modal_path_deleted,
};

fn conflict_path_choice_state() -> &'static ConflictModalState {
    static S: OnceLock<ConflictModalState> = OnceLock::new();
    S.get_or_init(|| ConflictModalState {
        ...
        message: "The spending path you were editing has been modified by another user.\nReload to see the latest changes, or keep your edits.".to_string(),
    })
}

fn render_conflict_modal_path_choice() -> Element<'static, DebugMessage> {
    let body = conflict_modal_view(conflict_path_choice_state()).map(|_| ());
    installer_with_modal(
        "Business installer — conflict modal (path modified, choice)",
        CONFLICT_MODAL_PATH,
        body,
    )
}

fn conflict_path_deleted_state() -> &'static ConflictModalState {
    static S: OnceLock<ConflictModalState> = OnceLock::new();
    S.get_or_init(|| ConflictModalState {
        conflict_type: ConflictType::PathDeleted,
        title: "Path deleted".to_string(),
        message: "The spending path you were editing was deleted by another user.".to_string(),
    })
}

fn render_conflict_modal_path_deleted() -> Element<'static, DebugMessage> {
    let body = conflict_modal_view(conflict_path_deleted_state()).map(|_| ());
    installer_with_modal(
        "Business installer — conflict modal (path deleted, info)",
        CONFLICT_MODAL_PATH,
        body,
    )
}

const TEMPLATE_HELP_MODAL_PATH: &str =
    "business_installer::views::modals::template_help::template_help_modal_view";

pub static ENTRY_TEMPLATE_HELP_MODAL: DebugPageEntry = DebugPageEntry {
    view: render_template_help_modal,
};

fn template_help_state() -> &'static TemplateHelpModalState {
    static S: OnceLock<TemplateHelpModalState> = OnceLock::new();
    S.get_or_init(|| TemplateHelpModalState::new("Acme treasury".to_string()))
}

fn render_template_help_modal() -> Element<'static, DebugMessage> {
    let body = template_help_modal_view(template_help_state()).map(|_| ());
    installer_with_modal(
        "Business installer — template help modal",
        TEMPLATE_HELP_MODAL_PATH,
        body,
    )
}
...
pub const INSTALLER_STACK: DebugStack = DebugStack {
    ...
    pages: &[
        ...
        &orgs::ENTRY_ORG_SELECT_EMPTY,
        // wallet select
        &wallets::ENTRY_WALLET_SELECT_WITH_WALLETS,
        &wallets::ENTRY_WALLET_SELECT_EMPTY,
        // template creation
        ...
        &keys::ENTRY_KEYS_MANAGER_SETUP,
        &keys::ENTRY_KEYS_READY,
        ...
        &ENTRY_CONFLICT_MODAL_PATH_CHOICE,
        &ENTRY_CONFLICT_MODAL_PATH_DELETED,
        &ENTRY_TEMPLATE_HELP_MODAL,
