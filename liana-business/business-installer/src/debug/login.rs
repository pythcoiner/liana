...
fn account_select_many_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            let accounts = (0..8)
                .map(|i| CachedAccount {
                    email: format!("user{i}iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiooooooooooooooooooooooooooooooooooiiiii@example.com"),
                    tokens: stub_tokens(),
                })
                .collect();
            s.views.login = Login::with_cached_accounts(accounts);
...
fn email_empty_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            s.views.login.on_update_email(String::new());
        }))
    })
    .0
}
