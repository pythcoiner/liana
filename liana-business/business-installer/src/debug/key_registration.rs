...
use crate::state::message::{HardwareWalletRequestId, Msg as InstallerMessage};
use crate::state::{
    views::xpub::{ModalStep as XpubModalStep, XpubEntryModalState, XpubInputSource},
    State, View,
};
use crate::views::xpub::modal::{details_view, xpub_modal_view};
...
fn insert_locked(s: &mut State, kind: DeviceKind, pairing_code: Option<&str>, id: &str) {
    let device: SigningDevice<InstallerMessage, HardwareWalletRequestId> = SigningDevice::Locked {
        id: id.to_string(),
...
fn insert_unsupported(
    ...
    let device: SigningDevice<InstallerMessage, HardwareWalletRequestId> =
...
fn xpub_partial_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            // first seeded key so `owned_keys` is non-empty, and set the xpub
            // on the first key only so `all_keys_set` is false.
            let mut template = s.app.template();
            if let Some(k) = template.keys.values_mut().next() {
    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            s.views
                ...
                .on_update_email("alice@example.com".to_string());
            let mut template = s.app.template();
            for key in template.keys.values_mut() {
...
fn xpub_modal_state_with_step(step: XpubModalStep) -> State {
    build_state(|s| {
        ...
        let mut modal = XpubEntryModalState::new(
            1,
            "Bobxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx".to_string(),
            None,
            Network::Bitcoin,
        );
        modal.step = step;
...
fn xpub_modal_details_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_details_state(Network::Testnet, |modal| {
            modal.input_source = Some(XpubInputSource::Device {
                kind: "coldcard".to_string(),
                fingerprint: "19608592".to_string(),
                version: None,
            });
...
fn render_xpub_modal_details() -> Element<'static, DebugMessage> {
    let modal = xpub_modal_details_state()
        .views
        .xpub
        .modal
        .as_ref()
        .expect("modal set");
    let body = details_view(modal).map(|_| ());
    installer_with_modal(
...
fn build_select_state(
    ...
    build_state(|s| {
        ...
        let mut modal = XpubEntryModalState::new(
            1,
            "Bobtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt".to_string(),
            None,
            Network::Bitcoin,
        );
...
fn xpub_modal_select_with_current_xpub_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            let mut modal = XpubEntryModalState::new(
                1,
                "Bobuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu".to_string(),
                Some(sample_xpub()),
                Network::Bitcoin,
            );
            modal.step = XpubModalStep::Select;
...
fn xpub_modal_select_multiple_devices_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(build_select_state(
            |_| {},
            |s| {
                insert_locked(s, DeviceKind::BitBox02, Some("ABC-123-XYZ"), "bb02-locked");
                insert_locked(s, DeviceKind::Jade, None, "jade-locked");
                insert_unsupported(
                    s,
                    DeviceKind::Coldcard,
...
fn render_xpub_modal_select_multiple_devices() -> Element<'static, DebugMessage> {
    let body = xpub_modal_view(xpub_modal_select_multiple_devices_state())
        ...
        .map(|_| ());
    installer_with_modal(
        "Business installer: xpub modal (Select, all device-state variants)",
        XPUB_MODAL_PATH,
        body,
    )
}
...
fn build_details_state(
    ...
    build_state(|s| {
        s.current_view = View::Xpub;
        let mut modal = XpubEntryModalState::new(1, "Boboooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo".to_string(), None, network);
        modal.step = XpubModalStep::Details;
        // A selected device makes `account_picker` render the account combobox
        // instead of only its label.
        modal.selected_device = Some(Fingerprint::from_str("19608592").expect("valid fingerprint"));
        setup_modal(&mut modal);
        s.views.xpub.modal = Some(modal);
    })
}
...
fn render_xpub_modal_details_fetching() -> Element<'static, DebugMessage> {
    let modal = xpub_modal_details_fetching_state()
        .views
        .xpub
        .modal
        .as_ref()
...
fn render_xpub_modal_details_fetch_error() -> Element<'static, DebugMessage> {
    let modal = xpub_modal_details_fetch_error_state()
        .views
        .xpub
        .modal
        .as_ref()
...
fn render_xpub_modal_details_fetch_success() -> Element<'static, DebugMessage> {
    let modal = xpub_modal_details_fetch_success_state()
        .views
        .xpub
        .modal
        .as_ref()
...
fn render_xpub_modal_details_wrong_network() -> Element<'static, DebugMessage> {
    let modal = xpub_modal_details_wrong_network_state()
        .views
        .xpub
        .modal
        .as_ref()
...
fn render_xpub_modal_details_account_5() -> Element<'static, DebugMessage> {
    let modal = xpub_modal_details_account_5_state()
        .views
        .xpub
        .modal
        .as_ref()
