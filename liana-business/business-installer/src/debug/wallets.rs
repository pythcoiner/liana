    ...
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            ...
            let entries: &[(&str, WalletStatus)] = &[
                ...
                (
                    "Looooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooocked wallet",
                    WalletStatus::Locked,
                ),
...
pub static ENTRY_WALLET_SELECT_EMPTY: DebugPageEntry = DebugPageEntry {
    view: render_wallet_select_empty,
};

/// A selected org with no wallets: `wallet_select_view` renders its empty
/// placeholder.
fn wallet_select_empty_state() -> &'static State {
    static S: OnceLock<StateCell<State>> = OnceLock::new();
    &S.get_or_init(|| {
        StateCell(build_state(|s| {
            s.app.global_user_role = Some(UserRole::WizardSardineAdmin);
...
fn render_wallet_select_empty() -> Element<'static, DebugMessage> {
    let body = wallet_select_view(wallet_select_empty_state()).map(|_| ());
    installer_chrome(
        "Business installer — wallet select (empty)",
        WALLET_SELECT_PATH,
        body,
    )
}
