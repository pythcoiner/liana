...
fn template_builder_with_status(role: UserRole, status: WalletStatus) -> State {
    build_state(|s| {
        s.current_view = View::TemplateEdit;
    ...
    &S.get_or_init(|| {
        StateCell({
            ...
            template.keys.insert(
                ...
                Key {
                    id: 0,
                    alias: "Wallet manaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaager".to_string(),
            ...
            template.primary_path = SpendingPath::new(true, 1, Vec::new());
...
// surrounding `State` keeps the default `seed_test_data` keys (3 keys) and
...
fn path_modal_recovery_timelock_duplicate_state() -> &'static State {
    ...
    &S.get_or_init(|| {
        StateCell(recovery_modal_state(EditPathModalState {
            is_primary: false,
            // Editing existing path index 0 (blocks 8760 in `seed_test_data`).
            // 3650 hours × BLOCKS_PER_HOUR(6) = 21900 blocks, which collides
            // with the second seeded path (`seed_test_data` uses 21900) → the
