...
pub fn cltv_align(target: u32) -> u32 {
    (target & !(CLTV_ALIGNMENT - 1)).saturating_add(CLTV_ALIGNMENT)
}
...
impl OXpub {
    ...
    pub fn validate(&self) -> Result<(), OXpubError> {
        if self.xkey.public_key == super::analysis::bip341_nums() {
...
pub enum TapPosition {
    ...
    InternalKey,
    /// Path occupies one or more tap-tree leaves. For [`Semantic::MultiWithMandatory`] this
...
/// renewable heights. Lifted into [`super::policy::PolicyError`] via `From`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub enum Locktime {
    ...
    None,
    /// `older(...)` / OP_CSV - a relative timelock.
...
/// `MultiWithMandatory`, or a key that fails [`OXpub::validate`]. Lifted into
/// [`super::policy::PolicyError`] via `From`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum SemanticError {
    /// `Multi.threshold` must be in `[1, keys.len()]`.
    InvalidThreshold { threshold: usize, key_count: usize },
    /// `MultiWithMandatory.threshold` must be in
...
pub enum Semantic {
    ...
    Or(Vec<Semantic>),
    /// A policy we couldn't classify into any known spending-path shape.
    Unknown {
...
impl Semantic {
    ...
    pub fn keys(&self) -> impl Iterator<Item = &OXpub> + '_ {
        let v: Vec<&OXpub> = match self {
            ...
            Semantic::Unknown { .. } | Semantic::Or(_) => vec![],
        };
        v.into_iter()
    }
    ...
    pub fn sig_count(&self) -> Option<usize> {
        match self {
            ...
            Semantic::MultiWithMandatory { threshold, .. } => Some(*threshold),
            Semantic::Unknown { .. } | Semantic::Or(_) => None,
        }
    }

    /// Structural sanity check: `Multi` / `MultiWithMandatory` thresholds must be in range,
    /// and every signer key (`OXpub`) must pass [`OXpub::validate`] (no NUMS, no zeroed
    /// chain code). Or / Unknown are structurally fine here (their compilability is a policy
    /// concern, not a semantic shape concern).
    pub fn validate(&self) -> Result<(), SemanticError> {
        for k in self.keys() {
            k.validate().map_err(SemanticError::InvalidKey)?;
        }
        match self {
            Semantic::Single(_) | Semantic::Or(_) | Semantic::Unknown { .. } => Ok(()),
    ...
    pub fn starting_index(&self, locktime: &Locktime) -> Option<u32> {
        ...
        match (locktime, self) {
            ...
            (Locktime::Relative(_), Semantic::MultiWithMandatory { .. }) => {
...
pub struct Path {
    ...
    /// for `MultiWithMandatory`); empty for `TapPosition::InternalKey` and for paths
...
impl Path {
    ...
    pub fn leaf_count(&self) -> usize {
        match &self.semantic {
            Semantic::Single(_) | Semantic::Multi { .. } => 1,
            Semantic::MultiWithMandatory {
                keys,
                mandatory_keys,
                threshold,
                ...
                let k = (threshold - m).min(n - (threshold - m));
    ...
    pub(super) fn set_start_index(
        &mut self,
        start: u32,
    ) -> Result<u32, super::policy::PolicyError> {
        ...
        for frag in fragments {
            let ms = frag.compile::<Tap>().map_err(|e| {
                super::policy::PolicyError::Descriptor(super::LianaDescError::Miniscript(e.into()))
            })?;
            self.leaves.push(ms);
        }
        Ok(cursor)
...
mod tests {
    ...
    fn semantic_keys_iteration() {
        let k1 = oxpub("[abcdef01]xpub6Eze7yAT3Y1wGrnzedCNVYDXUqa9NmHVWck5emBaTbXtURbe1NWZbK9bsz1TiVE7Cz341PMTfYgFw1KdLWdzcM1UMFTcdQfCYhhXZ2HJvTW/<0;1>/*");
        let k2 = oxpub("[abcdef02]xpub688Hn4wScQAAiYJLPg9yH27hUpfZAUnmJejRQBCiwfP5PEDzjWMNW1wChcninxr5gyavFqbbDjdV1aK5USJz8NDVjUy7FRQaaqqXHh5SbXe/<0;1>/*");
        let k3 = oxpub("[abcdef03]xpub6Bw79HbNSeS2xXw1sngPE3ehnk1U3iSPCgLYzC9LpN8m9nDuaKLZvkg8QXxL5pDmEmQtYscmUD8B9MkAAZbh6vxPzNXMaLfGQ9Sb3z85qhR/<0;1>/*");
        ...
        let s = Semantic::MultiWithMandatory {
    ...
    fn path_satisfaction_wu_starts_none() {
        let k = oxpub("[abcdef01]xpub6Eze7yAT3Y1wGrnzedCNVYDXUqa9NmHVWck5emBaTbXtURbe1NWZbK9bsz1TiVE7Cz341PMTfYgFw1KdLWdzcM1UMFTcdQfCYhhXZ2HJvTW/<4;5>/*");
        let p = Path::new(
            Semantic::Single(k),
            Locktime::None,
