...
def build_with_fontforge(glyphs: list[tuple[str, int, Path]], output_font_file: Path) -> None:
    try:
        import fontforge
    except ModuleNotFoundError as err:
        raise SystemExit(
            "fontforge is required to build liana-icons.ttf. "
...
def main() -> None:
    ...
    codepoints_path = base_dir / "codepoints.json"
    output_font_file = base_dir / "liana-icons.ttf"
    glyphs = iter_glyphs(svg_dir, codepoints_path)
    build_with_fontforge(glyphs, output_font_file)
