...
/// The card drawn when the caller sets no style. Only the quad fields are read, the text colour
/// never is.
fn card_style(theme: &Theme, status: button::Status) -> button::Style {
    let card = crate::theme::card::button_simple(theme);
    button::Style {
        background: card.background,
        text_color: card.text_color.unwrap_or_default(),
        border: card.border,
    ...
    fn update(
        ...
        match event {
            Event::Mouse(mouse::Event::CursorMoved { .. } | mouse::Event::CursorLeft) => {
                let hovered = cursor.is_over(zone);
                if hovered != state.hovered {
                    state.hovered = hovered;
                    shell.request_redraw();
                }
            }
            // A child may have used the press already, e.g. to scroll.
            Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Left))
                if cursor.is_over(zone) && !shell.is_event_captured() =>
            {
                match &self.on_toggle {
                    Some(on_toggle) => shell.publish(on_toggle()),
                    None => state.expanded = !state.expanded,
                }
