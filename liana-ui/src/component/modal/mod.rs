...
use crate::widget::{Button, CheckBox, Column, Element, PickList, RowExt, SpaceExt, Text};
    ...
    A: Display + 'a,
{
    ...
    let row = row![icon, designation, Space::fill_width()]
        .push_maybe(Option::<Element<'a, M>>::from(status))
        .align_y(Vertical::Center)
        .spacing(H_SPACING);
    button::device(row, on_press)
}
    ...
    Message: Clone + 'static,
{
    ...
    let body = if let Some(status) = status {
        column![title, fingerprint, status]
    } else {
        column![title, fingerprint]
    }
    .spacing(2)
    .width(Length::Fill);
