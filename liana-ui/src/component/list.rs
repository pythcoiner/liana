...
pub enum DeviceStatus {
    ...
    AlreadyUsed,
    /// Show the device fingerprint.
    Fingerprint(Fingerprint),
    /// Show the device fingerprint with a chevron (the device is selectable).
    ...
    Registered,
    Selected,
    /// A standalone warning shown as a tooltip.
    Warning(&'static str),
}
...
pub fn entry_paste_xpub<'a, M: Clone + 'a>(
    ...
    list_entry_row_static(
        Some(badge::tile(TileName::Paste).into()),
...
pub fn entry_wallet<'a, M: Clone + 'a>(
    ...
    list_entry_row(
        Some(badge::tile(TileName::Wallet).into()),
...
fn key_tile(kind: EntryKeyKind) -> TileName {
    match kind {
        EntryKeyKind::Internal => TileName::KeyInternal,
        EntryKeyKind::External => TileName::KeyExternal,
        EntryKeyKind::SafetyNet => TileName::KeyService,
    }
}
