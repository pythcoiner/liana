...
fn render_receive() -> Element<'static, DebugMessage> {
    let body = receive_view::receive(&[], empty_labels(), false, empty_label_forms(), true, false)
        .map(|_| ());
...
use crate::app::state::{FeeMode, Recipient};
use crate::app::view::spend::{create_spend_tx, recipient_view};

fn create_spend_recipients() -> &'static [Recipient] {
    use liana_ui::component::form::Value;
    let value = |v: &str| Value {
        value: v.to_string(),
        warning: None,
        valid: true,
    };
    static R: OnceLock<Vec<Recipient>> = OnceLock::new();
    R.get_or_init(|| {
        vec![Recipient {
            address: value(SAMPLE_ADDRESSES[0]),
            amount: value("0.001"),
            label: value("Rent"),
            ..Default::default()
        }]
...
fn self_spend_cache() -> &'static crate::app::cache::Cache {
    static CACHE: OnceLock<PanelsCell<crate::app::cache::Cache>> = OnceLock::new();
    &CACHE
        .get_or_init(|| {
            let mut c = crate::app::cache::Cache::default();
            c.daemon_cache.blockheight = 800_000;
            PanelsCell(c)
        })
        .0
}

fn self_spend_label_txid() -> Txid {
    Txid::from_str("1".repeat(64).as_str()).expect("valid hex")
}

fn self_spend_coins() -> &'static Vec<(Coin, bool)> {
    static COINS: OnceLock<Vec<(Coin, bool)>> = OnceLock::new();
    COINS.get_or_init(|| {
        let txid0 = Txid::from_str("0".repeat(64).as_str()).expect("valid hex");
        let txid1 = self_spend_label_txid();
        let spent_txid = Txid::from_str("2".repeat(64).as_str()).expect("valid hex");
        let mk = |amount: u64,
                  txid: Txid,
                  vout: u32,
                  addr_idx: usize,
                  block_height: Option<i32>,
                  spend_info: Option<lianad::commands::LCSpendInfo>,
                  selected: bool| {
            (
                Coin {
                    amount: Amount::from_sat(amount),
                    outpoint: OutPoint { txid, vout },
                    address: parse_addr(SAMPLE_ADDRESSES[addr_idx]),
                    block_height,
                    derivation_index: ChildNumber::from_normal_idx(vout).unwrap(),
                    spend_info,
                    is_immature: false,
                    is_change: false,
                    is_from_self: false,
        ...
        vec![
            // long-form sequence pill, selected
            mk(931_877_204, txid0, 0, 0, Some(798_000), None, true),
            // "~2 days" pill, outpoint-keyed label
            mk(52_050_244, txid0, 1, 1, Some(796_150), None, false),
            // "Today" pill, txid-keyed label (rendered as "From <label>")
            mk(15_675_130, txid1, 0, 2, Some(796_000), None, false),
            // "Available" pill (recovery already reachable)
            mk(9_931_877_204, txid0, 3, 3, Some(795_968), None, false),
            // unconfirmed pill, selected, no label
            mk(180_000, txid0, 4, 0, None, None, true),
            // unconfirmed pill, unselected, outpoint-keyed label
            mk(95_000, txid0, 5, 1, None, None, false),
            // spent pill
            mk(
                3_000_000,
                spent_txid,
                ...
                2,
                Some(799_500),
                Some(lianad::commands::LCSpendInfo {
                    txid: spent_txid,
                    height: Some(799_900),
                }),
                false,
...
fn self_spend_labels() -> &'static HashMap<String, String> {
    static M: OnceLock<HashMap<String, String>> = OnceLock::new();
    M.get_or_init(|| {
        let coins = self_spend_coins();
        let mut m = HashMap::new();
        // entry #2: outpoint-keyed label
        m.insert(
            coins[1].0.outpoint.to_string(),
            "Cold storage top-up".to_string(),
        );
        // entry #3: txid-keyed label, rendered as "From <label>"
        m.insert(
            self_spend_label_txid().to_string(),
            "Exchange withdrawal".to_string(),
        );
        // entry #6: outpoint-keyed label on an unconfirmed coin
        m.insert(
            coins[5].0.outpoint.to_string(),
            "Pending deposit Pending deposit Pending deposit Pending deposit Pending deposit Pending deposit Pending deposit ".to_string(),
        );
        m
    })
}
