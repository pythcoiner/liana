...
fn view() -> Element<'static, DebugMessage> {
    ...
    let rows: Vec<(&'static str, Element<'static, DebugMessage>)> = vec![
        ("Form::new(\"placeholder\", &Value::default(), |_| ())", Form::new("placeholder", &V_EMPTY, on_change).into()),
        ("Form::new(\"placeholder\", <hello>, |_| ())",           Form::new("placeholder", &V_HELLO, on_change).into()),
        ("Form::new(\"placeholder\", <invalid>, |_| ())",         Form::new("placeholder", &V_INVALID, on_change).into()),
        ("Form::new_disabled(\"placeholder\", <hello>)",          Form::new_disabled("placeholder", &V_HELLO).into()),
        ("Form::new_disabled(\"placeholder\", <empty>)",          Form::new_disabled("placeholder", &V_EMPTY).into()),
        ("Form::new_trimmed(\"placeholder\", <hello>, |_| ())",   Form::new_trimmed("placeholder", &V_HELLO, on_change).into()),
        ("Form::new_amount_btc(\"0.0\", <btc>, |_| ())",          Form::new_amount_btc("0.0", &V_BTC, on_change).into()),
        ("Form::new(...).padding(20)",                            Form::new("placeholder", &V_EMPTY, on_change).padding(20).into()),
        ("Form::new(...).size(28)",                               Form::new("placeholder", &V_HELLO, on_change).size(28).into()),
        ("Form::new(...).warning(\"...\")",                       Form::new("placeholder", &V_INVALID, on_change).warning("Explicit warning").into()),
        ("Form::new(...).on_submit(())",                          Form::new("placeholder", &V_HELLO, on_change).on_submit(()).into()),
        ("Form::new(...).label(\"Amount (BTC)\")",                Form::new("placeholder", &V_EMPTY, on_change).label("Amount (BTC)").into()),
        ("Form::new(...).label(\"Address\")",                     Form::new("address", &V_HELLO, on_change).label("Address").into()),
    ];
